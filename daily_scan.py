"""
daily_scan.py  —  Full automated daily scan: $5-$20 US stocks, 30-point scoring
Runs every day at 3:00 PM (Saudi time).
"""
...
<head>
<meta charset="UTF-8">
<title>US Market Scan $5-$20 | {DATE_STR}</title>
<style>
...
<body>
<div class="header">
  <h1>&#127482;&#127480; US MARKET SCAN &mdash; $5 to $20 STOCKS</h1>
...
function sortTable(col) {{
  ...
  table.setAttribute('data-sort-dir', asc ? 'desc' : 'asc');
    ...
    html_path = os.path.join(BASE, f"US_Mid_Scan_{TODAY}.html")
    with open(html_path, 'w', encoding='utf-8') as f:
        f.write(html)
    size_kb = os.path.getsize(html_path) // 1024
    log(f"  HTML report saved: US_Mid_Scan_{TODAY}.html ({size_kb} KB)")
    return html_path

if __name__ == '__main__':
    log("=" * 60)
    log(f"MID-RANGE SCAN STARTED — {DATE_STR}")
    log("=" * 60)
    t0 = time.time()
    try:
        clear_stale()
